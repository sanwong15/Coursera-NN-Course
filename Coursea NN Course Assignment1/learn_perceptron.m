...
for i=1:num_neg_examples
    ...
    if (activation >= 0) % which means you have an error as "activation" is suppose to be <0 to correctly pretect Class 0
        % Need to update weight for those who predict wrong
        %YOUR CODE HERE
            % w = w + learningRate*(t-predicted)*x (neg_example means t =
            % 0, for activate >=0. Therefore, t-predicted is 0-1
            % i.e: w = w -learning*x
        w = w - x;
    end
end
